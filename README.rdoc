...
The Aesia library components set have one dependence regarding the spell checking, and since conditionnal compilation can't be used with import statements, event if you don't use the spelling features you should link to the Adobe Spelling SDK when compiling.
...
At that time, a whole part of the components core objects is initialized (in fact, most of the components core elements works on a lazy init basis) such as the +RepaintManager+ and the +SkinManager+.
...
Let's take a look to the first one. Below, an example of a compilation command line with all the conditionnal compilation constants defined as command line arguments : 
	...
	mxmlc -source-path+=PATH_TO_THE_AESIA_LIB \
